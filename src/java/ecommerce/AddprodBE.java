...
//out.println("<h1>Servlet AddAttdBE at " + request.getContextPath() + "</h1>");
            ...
            String sql = "insert into PRODUCT(IMG,PNAME,CATEGORY,PPRICE,QUANTITY) values(?,?,?,?,?)";
        ...
        out.println("        window.location.href = 'admin.html';"); // Change the URL here
        out.println("    } else {");
        out.println("        window.location.href = 'AddprodBE.html';");
        out.println("    }");
        out.println("</script>");
            }
            con.close();
